...
export default function Home() {
  ...
  async function join(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus("loading");
    try {
      const res = await fetch("/api/waitlist", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(data)) });
    ...
    <main className={paused ? "motion-paused" : ""}>
      ...
      <section className="hero" id="main-content">
        ...
        <div className="radar-scene" aria-label="Animated illustration of nearby picks, for demonstration">
          ...
          <div className="float-card card-one"><div className="card-thumb"><img src="/images/city-night.jpg" alt="Friends outside a warmly lit neighborhood music venue"/></div><div className="card-body"><div className="micro lime">LIVE MUSIC · 8 MIN</div><strong>Tonight has a soundtrack.</strong><div className="card-meta"><span>Walk-ins welcome</span><span className="go-icon"><ArrowUpRight size={18}/></span></div></div></div>
      ...
      <section className="section possibilities">
        ...
        <div className={`outing-preview ${picks[active].className}`} aria-live="polite"><img src="/images/city-night.jpg" alt="Friends catching up on a city sidewalk outside a music venue"/><div className="photo-shade"/><div className="photo-top"><span className="photo-label">THE CITY IS BETTER IN PERSON.</span><span>↗</span></div><div className="outing-info"><div className="outing-chips"><span>{picks[active].time}</span><span>{picks[active].cost}</span></div><h3>{picks[active].title}</h3><p>{picks[active].note}</p></div></div>
      </section>
      ...
      <section className="waitlist section" id="waitlist"><div><p className="eyebrow">YOUR NEXT “GLAD I WENT” STARTS HERE.</p><h2>BE FIRST.<br/>BE <span>OUT.</span></h2><p>We’re starting small, one neighborhood at a time.<br/>Tell us where you are. We’ll let you know when it’s your turn.</p></div><div className="signup-panel">{status === "success" ? <div className="success" role="status"><span className="success-check"><Check size={35}/></span><h3>You’re on the list.</h3><p>We’ll email you when OutRN is ready for your neighborhood. Until then, go live a little.</p></div> : <form onSubmit={join}><h3>Get on the list.</h3><label htmlFor="email">Email address</label><input id="email" name="email" type="email" autoComplete="email" placeholder="you@example.com" maxLength={254} required disabled={status==="loading"}/><label htmlFor="neighborhood">Your neighborhood / city</label><input id="neighborhood" name="neighborhood" autoComplete="address-level2" placeholder="e.g. Lower East Side, NYC" maxLength={120} minLength={2} required disabled={status==="loading"}/><div className="honeypot" aria-hidden="true"><label htmlFor="website">Website</label><input id="website" name="website" tabIndex={-1} autoComplete="off"/></div><button className="button" disabled={status==="loading"} type="submit">{status==="loading" ? "Saving your spot…" : "Let me know when it’s on."}<ArrowUpRight size={22}/></button>{status==="error" && <p role="alert" className="form-error">{message}</p>}<p className="privacy-note">By joining, you agree to receive OutRN launch updates. We’ll store your email and neighborhood for that purpose. Unsubscribe anytime by replying to an update.</p></form>}</div></section>
