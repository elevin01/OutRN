...
const picks = [
  { title: "There’s jazz downstairs.", label: "Something loud", category: "LIVE MUSIC", time: "8 min walk", cost: "$15 at the door", icon: Music2, note: "Small room. Live band. You can just walk in.", aside: "Stay for one song. Or five." },
  { title: "The gallery’s still open.", label: "Something different", category: "ART & OTHER THINGS", time: "12 min walk", cost: "Free", icon: Sparkles, note: "A pop-up show, a few new faces, no ticket needed.", aside: "You don’t have to know about art." },
  { title: "Two seats at the counter.", label: "Something to eat", category: "FOOD & DRINK", time: "5 min walk", cost: "Around $20", icon: MapPin, note: "Dinner without the group chat or the reservation.", aside: "Going alone counts, by the way." },
];
    ...
    <main className={paused ? "motion-paused" : ""}>
      ...
      <section className="hero" id="main-content">
        ...
        <div className="radar-scene" aria-label="Animated preview of nearby picks. These are examples, not live listings.">
          <div className="radar-top micro"><Radio size={15}/> A FEW BLOCKS FROM YOU</div>
          ...
          <div className="float-card card-one"><div className="card-thumb"><img src={cityImage} alt="An illustration of friends outside a neighborhood music venue"/></div><div className="card-body"><div className="micro lime">LIVE MUSIC · 8 MIN WALK</div><strong>There’s jazz downstairs.</strong><div className="card-meta"><span>$15 at the door. Walk right in.</span><span className="go-icon"><ArrowUpRight size={18}/></span></div></div></div>
          <div className="float-card card-two"><span className="small-icon"><Sparkles size={22}/></span><div><div className="micro">12 MIN WALK · FREE</div><strong>The gallery’s still open.</strong><p>No, you don’t need a ticket.</p></div></div>
          <div className="radar-stamp"><span>TONIGHT’S PLAN</span><strong>WHY<br/>NOT?</strong><ArrowUpRight size={27}/></div>
      ...
      <div className="ticker" aria-hidden="true"><div>{Array.from({length:4},(_,i)=><span key={i}>“ONE MORE EPISODE” CAN WAIT <i>✳</i> SHOES ON <i>✳</i> WE’LL FIGURE IT OUT ON THE WAY <i>✳</i></span>)}</div></div>
      ...
      <footer><a className="wordmark" href="#" aria-label="Back to top">OUT<span>RN</span><i>↗</i></a><p>See you out there.</p><span className="micro">© {new Date().getFullYear()} OUT RN</span></footer>
    </main>
  );
}
